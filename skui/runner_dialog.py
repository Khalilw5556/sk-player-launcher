...
class RunnerDialog(BaseFramelessDialog):
    def __init__(self, game, parent=None):
        ...
        self.CUSTOM_RUNNERS_DIR = os.path.join(RUNNERS_DIR, "custom")
        if not os.path.exists(self.CUSTOM_RUNNERS_DIR):
            os.makedirs(self.CUSTOM_RUNNERS_DIR, exist_ok=True)
    ...
    def apply_styles(self):
        self.content_area.setStyleSheet("""
            ...
            QLabel { background: transparent; border: none; color: #888; font-weight: bold; font-size: 12px; }
            QComboBox { background-color: #121212; color: #eee; border: 1px solid #333; border-radius: 8px; padding: 8px; font-size: 14px; }
            QComboBox::drop-down { border: none; }
            QProgressBar { background-color: #050505; border: 1px solid #333; border-radius: 10px; text-align: center; color: white; font-weight: bold; }
            QProgressBar::chunk { background-color: #27ae60; border-radius: 9px; }
            QPushButton { background: #181818; border-radius: 10px; padding: 12px; font-weight: bold; color: white; border: 1px solid #333; }
            QPushButton:hover { background: #333; }
            QPushButton#apply_btn { background-color: #27ae60; border: none; }
    ...
    def setup_ui(self):
        ...
        self.type_combo.addItems(["System", "Wine", "Proton", "Custom"])
    ...
    def load_custom_runners(self):
        try:
            if not os.path.exists(self.CUSTOM_RUNNERS_DIR):
                os.makedirs(self.CUSTOM_RUNNERS_DIR)

            folders = [f for f in os.listdir(self.CUSTOM_RUNNERS_DIR)
                       if os.path.isdir(os.path.join(self.CUSTOM_RUNNERS_DIR, f))]

            if not folders:
                self.status_lbl.setText("No custom runners found in data/runners/custom")
                self.status_lbl.setStyleSheet("color: #e74c3c;")
                self.apply_btn.setEnabled(False)
            else:
                for folder in folders:
                    self.ver_combo.addItem(folder, {"name": folder, "custom": True})

                self.status_lbl.setText(f"Found {len(folders)} custom runners.")
                self.status_lbl.setStyleSheet("color: #27ae60;")
                self.apply_btn.setEnabled(True)

        except Exception as e:
            self.on_error(f"Failed to load custom runners: {str(e)}")
    ...
    def handle_apply(self):
        ...
        if r_type == "Custom" or is_runner_installed(r_type, v_name):
    ...
    def start_download(self, r_type, v_data):
        ...
        def download_thread():
            try:
                ...
                self.signals.status.emit("Extracting files...")
